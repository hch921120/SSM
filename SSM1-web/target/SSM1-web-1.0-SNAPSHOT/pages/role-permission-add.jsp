...
<div>
    <form id="rolePermissionForm" action="/role/permission/add" method="post">
        <table id="dataList"
            ...
            <tbody>
            <c:forEach items="${rolepermissons}" var="permission">
                <tr>
                    <td><input name="ids" <c:if test="${fn:contains(rolePermisson,'['.concat(permission.id).concat(']'))}">checked="checked"</c:if> type="checkbox" value="${permission.id}"></td>
                    <td>${permission.id}</td>
